...
BitcoinGUI::BitcoinGUI(QWidget *parent):
    ...
    setWindowTitle(tr("Spark") + " " + tr("Wallet"));
    qApp->setStyleSheet("QMainWindow { background:#1f1f1f; font-family:'Segoe UI'; } #toolbar2 { border:none;width:30px; background:#cc2f24; }");
#ifndef Q_OS_MAC
    ...
    actionConvertCurrency->setToolTip("Click here to convert your Spark to USD and BTC.  Spark price is pegged to BTC/SPARK market and BTC/USD market on Coinbase.");
...
void BitcoinGUI::createActions() {
    ...
    sendCoinsAction = new QAction(QIcon(":/icons/null"), tr("&Send Spark"), this);
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
    toolbarsend2->addAction(sendCoinsAction);
    tabGroup->addAction(sendCoinsAction);

    receiveCoinsAction = new QAction(QIcon(":/icons/null"), tr("&Receive Spark"), this);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel) {
    ...
    if (clientModel) {
        ...
        if (clientModel->isTestNet()) {
            ...
            if (trayIcon) {
                trayIcon->setToolTip(tr("Spark client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon() {
    ...
    trayIcon->setToolTip(tr("Spark client"));
...
void BitcoinGUI::setNumConnections(int count) {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Spark network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event) {
    if (event->mimeData()->hasUrls()) {
        ...
        if (nValidUrisFound) {
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Spark address or malformed URI parameters."));
        }
    }
